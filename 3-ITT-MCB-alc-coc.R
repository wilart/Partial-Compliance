...
dat <- dat_for_analysis_4 %>% filter(s == 1) %>% rbind(dat_for_analysis_4 %>% filter(s == 1) %>% mutate(a2 =-a2), dat_for_analysis_4 %>% filter(s==0))

a1 <- dat$a1
s <- dat$s
a2 <- dat$a2
y <- log(dat$outcome+0.5)
GH <- dat$GH_baseline
TR <- dat$higheduce
...
)+  scale_shape_manual(values = (c("EDTR 1" = 15,"EDTR 2" = 16,"EDTR 3" = 17,"EDTR 4" = 18)) )+scale_colour_manual(values =(c("EDTR 1" = cbPalette[1],"EDTR 2" =cbPalette[2],"EDTR 3" = cbPalette[3],"EDTR 4" = cbPalette[4]))  )+ggtitle("MCB Plot ENGAGE Overall ITT Alcohol + Cocaine")+scale_y_continuous(limits = c(-6.5,5))+theme(plot.title = element_text(size=22),axis.title = element_text(size=22),
                                                                                                                                                                                                                                                                                                                  axis.text = element_text(size=22)
                                                                                                                                                                                                                                                                                                                  )
g_plot

#pdf("Real-data-MCB-plot-overall-24-weeks-3-10-29-21.pdf",width=8,height=8)
g_plot
dev.off()
##############################
########################
