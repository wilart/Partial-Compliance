...
# for an ENGAGE-type study such that the stage-1 potential compliances are generated from a Gaussian copula with truncated normal margins
...
ComplianceSimulate <- function(n, n_sim){
  ...
  for (i in 1:n_sim){
    ...
    y1 <- 2.5 +a1*(-0.8*D1+0.4*D2)+a2*(-0.2*D3*(a1!=-1)+0.5*D4*(a1!=1))*(1-s)+0.3*X1+0.6*X2-0.7*X3
    
   
    y <- MASS::rnegbin(n,exp(y1),0.4)
...
set.seed(136456)
sim_1truncnorm_160_zero_inflated<- ComplianceSimulate(n=160,n_sim=400)
...
zero_inflated_median <- apply(do.call(rbind,lapply(sim_1truncnorm_160_zero_inflated,function(x)coef(lm(y~I(a1*D1)+I(a1*D2)+I(a2*D3*(a1!=-1))+I(a2*D4*(a1!=1))+X1+X2+X3,data=x)))),2,median)
(round(colMeans(do.call(rbind,lapply(1:400,function(x) (apply(MSM_160_zero_inflated[[x]][[1]],2,median))))),2))
