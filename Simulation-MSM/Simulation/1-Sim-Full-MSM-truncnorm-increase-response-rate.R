...
#We shrink some of the treatment sequence sample sizes by increasing the response rate.
library(copula)
library(truncnorm)
...
ComplianceSimulate <- function(n, n_sim){
  ...
  for (i in 1:n_sim){
    ...
    s <- rbinom(n,1,expit(1.3+a1*(0.5*D1+0.7*D2)-0.5+0.2*X1-0.2*X3))
...
set.seed(38265)
sim_1truncnorm_200_higher_response_rate<- ComplianceSimulate(n=200,n_sim=400)
