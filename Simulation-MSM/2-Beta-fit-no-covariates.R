#Fit non-parametric Dirichlet process mixture model for potential compliance and parametric outcome model (marginal structural model)
#For ENGAGE-type simulated SMART study
#Does not use baseline covariates in potential compliance joint distribution, but does use them in marginal structural model
...
computeZresponders <- function(n, H, W_current, D1_current, D2_current, eta_current, sigma_current) {
  ...
  # D1_current (D_{1,+1}) and D2_current (D_{1,-1}) correspond to stage-1 potential compliances 
  ...
  for (h in 1:H) {
    dens[,h] <- tmvtnorm::dtmvnorm(cbind(D1_current,
                                         D2_current), eta_current[h,], sigma_current[[h]],c(0,0),c(1,1))
  }
...
#################
computeZnonresponders <- function(n,H,W_current,D1_current,D2_current,D5_current,eta_current, sigma_current) {
  ...
  # H: number of DP mixture components
  # W_current: current DP mixture weights
  ##  D1_current (D_{1,+1}) and D2_current (D_{1,-1}) correspond to stage-1 potential compliances while D5_current corresponds to the stage-2 potential compliance, 
  #either D_{2,+1} or D_{2,-1}
  # sigma_current: list (H elements) of current pre-truncation covariance matrix estimate for each DP mixture component
  ...
  for (h in 1:H) {
    dens[,h] <- tmvtnorm::dtmvnorm( cbind(D1_current,
                                          D2_current,
                                          D5_current), eta_current[h,], sigma_current[[h]],c(0,0,0),c(1,1,1))
  }
...
computeBetas <- function(niter,dat,H_responders,H_non_responders) {
  ...
  # dat: dataset of the form generated by 1-Sim-Full-MSM-beta.R
  ...
  for (j in 1:niter) {#Each draw imputes the missing potential compliances after fitting their joint posterior
    ...
    for (i in 1:length(which(s==1))) {
      ...
      if (a1[which(s==1)][i]==1) {
        ...
        cond_mean <- condMVN(eta_current_responders[Z_current_responders[i],],sigma_current_responders[[Z_current_responders[i]]],dependent.ind = 2,given.ind = c(1),c(D1_current[which(s==1)][i]),check.sigma = F)$condMean
        cond_var <- condMVN(eta_current_responders[Z_current_responders[i],],sigma_current_responders[[Z_current_responders[i]]],dependent.ind = 2,given.ind = c(1),c(D1_current[which(s==1)][i]),check.sigma = F)$condVar
      ...
      if (a1[which(s==1)][i]==-1) {
        ...
        cond_mean <- condMVN(eta_current_responders[Z_current_responders[i],],sigma_current_responders[[Z_current_responders[i]]],dependent.ind = 1,given.ind = c(2),c(D2_current[which(s==1)][i]),check.sigma = F)$condMean
        cond_var <- condMVN(eta_current_responders[Z_current_responders[i],],sigma_current_responders[[Z_current_responders[i]]],dependent.ind = 1,given.ind = c(2),c(D2_current[which(s==1)][i]),check.sigma = F)$condVar
    ...
    for (i in 1:length(which(s==0&a1==1))){
      ...
      if (a1[which(s==0&a1==1)][i]==1&a2[which(s==0&a1==1)][i]==1) {
        ...
        #Conditional mean and variance of full conditional for unobserved compliance, D2
        cond_mean <- condMVN(eta_current_non_responders3[Z_current_non_responders3[i],],sigma_current_non_responders3[[Z_current_non_responders3[i]]],dependent.ind = 2,given.ind = c(1,3),c(D1_current[which(s==0&a1==1)][i],D3_current[which(s==0&a1==1)][i]),check.sigma = F)$condMean
        cond_var <- condMVN(eta_current_non_responders3[Z_current_non_responders3[i],],sigma_current_non_responders3[[Z_current_non_responders3[i]]],dependent.ind = 2,given.ind = c(1,3),c(D1_current[which(s==0&a1==1)][i],D3_current[which(s==0&a1==1)][i]),check.sigma = F)$condVar
      ...
      if (a1[which(s==0&a1==1)][i]==1&a2[which(s==0&a1==1)][i]==-1) {
        ...
        cond_mean <- condMVN(eta_current_non_responders3[Z_current_non_responders3[i],],sigma_current_non_responders3[[Z_current_non_responders3[i]]],dependent.ind = 3,given.ind = c(1,2),c(D1_current[which(s==0&a1==1)][i],D2_current[which(s==0&a1==1)][i]),check.sigma = F)$condMean
        cond_var <-  condMVN(eta_current_non_responders3[Z_current_non_responders3[i],],sigma_current_non_responders3[[Z_current_non_responders3[i]]],dependent.ind = 3,given.ind = c(1,2),c(D1_current[which(s==0&a1==1)][i],D2_current[which(s==0&a1==1)][i]),check.sigma = F)$condVar
    ...
    for (i in 1:length(which(s==0&a1==-1))){
      ...
      if (a1[which(s==0&a1==-1)][i]==-1&a2[which(s==0&a1==-1)][i]==1) {
        ...
        cond_mean <- condMVN(eta_current_non_responders4[Z_current_non_responders4[i],],sigma_current_non_responders4[[Z_current_non_responders4[i]]],dependent.ind = 1,given.ind = c(2,3),c(D2_current[which(s==0&a1==-1)][i],D4_current[which(s==0&a1==-1)][i]),check.sigma = F)$condMean
        cond_var <- condMVN(eta_current_non_responders4[Z_current_non_responders4[i],],sigma_current_non_responders4[[Z_current_non_responders4[i]]],dependent.ind = 1,given.ind = c(2,3),c(D2_current[which(s==0&a1==-1)][i],D4_current[which(s==0&a1==-1)][i]),check.sigma = F)$condVar
      ...
      if (a1[which(s==0&a1==-1)][i]==-1&a2[which(s==0&a1==-1)][i]==-1) {
        ...
        D2_current[which(s==0&a1==-1)][i] = D2[which(s==0&a1==-1)][i]
        #Conditional mean and variance of full conditional for unobserved compliance
        cond_mean <- condMVN(eta_current_non_responders4[Z_current_non_responders4[i],],sigma_current_non_responders4[[Z_current_non_responders4[i]]],dependent.ind = 3,given.ind = c(1,2),c(D1_current[which(s==0&a1==-1)][i],D2_current[which(s==0&a1==-1)][i]),check.sigma = F)$condMean
        cond_var <- condMVN(eta_current_non_responders4[Z_current_non_responders4[i],],sigma_current_non_responders4[[Z_current_non_responders4[i]]],dependent.ind = 3,given.ind = c(1,2),c(D1_current[which(s==0&a1==-1)][i],D2_current[which(s==0&a1==-1)][i]),check.sigma = F)$condVar
    ...
    for (h in 1:H_responders) {
      if ((sum(Z_current_responders==h))>1) {
        ...
        eta_prop_h <- MASS::mvrnorm(1,colMeans(cbind(D1_current[which(s==1)][which(Z_current_responders==h)],
                                                     D2_current[which(s==1)][which(Z_current_responders==h)])),sigma_current_responders[[h]]/sum(Z_current_responders==h))
        #############
        ##############
        accept_prob <- 1    
    ...
    for (h in 1:H_non_responders) {
      if ((sum(Z_current_non_responders3==h))>1) {
        ...
        )),sigma_current_non_responders3[[h]]/sum(Z_current_non_responders3==h))
        #############
        ##############
        accept_prob <- 1    
    ...
    for (h in 1:H_non_responders) {
      if ((sum(Z_current_non_responders4==h))>1) {
        ...
        )),sigma_current_non_responders4[[h]]/sum(Z_current_non_responders4==h))
        #############
        ##############
        accept_prob <- 1    
    ...
    for (h in 1:H_responders) {
      if (sum(Z_current_responders==h)>1) {
        ...
        var_temp <- crossprod((cbind(D1_current[which(s==1)][which(Z_current_responders==h)],
                                     D2_current[which(s==1)][which(Z_current_responders==h)])-matrix(eta_current_responders[h,],nrow=length(which(Z_current_responders==h)),ncol=2,byrow=T)),((cbind(D1_current[which(s==1)][which(Z_current_responders==h)],D2_current[which(s==1)][which(Z_current_responders==h)])-matrix(eta_current_responders[h,],nrow=length(which(Z_current_responders==h)),ncol=2,byrow=T))))
    ...
    for (h in 1:H_non_responders) {
      if (sum(Z_current_non_responders3==h)>1) {
        ...
        var_temp <- crossprod((cbind(D1_current[which(s==0&a1==1)][which(Z_current_non_responders3==h)],
                                     D2_current[which(s==0&a1==1)][which(Z_current_non_responders3==h)],
                                     D3_current[which(s==0&a1==1)][which(Z_current_non_responders3==h)]
        )-matrix(eta_current_non_responders3[h,],nrow=length(which(Z_current_non_responders3==h)),ncol=3,byrow=T)),((cbind(D1_current[which(s==0&a1==1)][which(Z_current_non_responders3==h)],D2_current[which(s==0&a1==1)][which(Z_current_non_responders3==h)],D3_current[which(s==0&a1==1)][which(Z_current_non_responders3==h)])-matrix(eta_current_non_responders3[h,],nrow=length(which(Z_current_non_responders3==h)),ncol=3,byrow=T))))
    ...
    #Draw mixture component indicators
    Z_current_non_responders3 <- computeZnonresponders(length(which(s==0&a1==1)),H_non_responders,W_current_non_responders3,D1_current[which(s==0&a1==1)],
                                                       ...
                                                       eta_current_non_responders3,sigma_current_non_responders3)
    Z_current_non_responders4 <- computeZnonresponders(length(which(s==0&a1==-1)),H_non_responders,W_current_non_responders4,D1_current[which(s==0&a1==-1)],
                                                       ...
                                                       eta_current_non_responders4,sigma_current_non_responders4)
    Z_current_responders <- computeZresponders(length(which(s==1)),H_responders,W_current_responders,D1_current[which(s==1)],D2_current[which(s==1)],eta_current_responders,sigma_current_responders)
    ...
    #a2[s==1] <- 0
    X_seq_overall <- data.frame(cbind(1,a1*D1_current,a1*D2_current,D3_current,D4_current,X1,X2,X3,D1_current,D2_current,D3_current,D4_current,a1,a2,s,y))
    ...
    beta_seq_mean_overall <- solve(t(X_seq_overall)%*%diag(w_residual2)%*%X_seq_overall)%*%t(X_seq_overall) %*%diag(w_residual2)%*% y2
    
    #Compute RSS for drawing from outcome variance
    RSS_seq_1 <- t(y2-X_seq_overall%*%beta_seq_mean_overall)%*%diag(w_residual)%*%(y2-X_seq_overall%*%beta_seq_mean_overall)
